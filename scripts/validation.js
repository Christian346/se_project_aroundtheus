...
function showInputError(individualForm, inputEl, {inputErrorClass , errorClass}) {
...
function hideInputError(individualForm, inputEl, { inputErrorClass, errorClass}) {
  const errorMessageEl = individualForm.querySelector(`#${inputEl.id}-error`)
...
function disableButton(){//adds class to disable it

}

function enableButton(){//removes it

}
//also escape and overlay click
...
/*
  ...
  inputEls.forEach((inputEl)=>{
     ...
     */
  if(/*foundInvalid*/hasInvalidInput(inputEls)){
...
function setEventListeners(individualForm, configObj) {
  ...
  const {inputSelector} = configObj // take out a property and create a variable called inputselector
